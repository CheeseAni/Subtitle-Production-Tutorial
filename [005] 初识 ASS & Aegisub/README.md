...
`YCbCr Matrix` 一定要设为与视频的颜色一致（一般是 TV.709），否则在 Aegisub 里预览的视频会偏色，但不会导致字幕filter渲染出来的字幕颜色改变，具体原因我也不清楚。  
如果预览的视频偏色，在这个预览视频中取色制作屏幕字的话，屏幕字就会偏色。  
...
如果渲染 ASS 的窗口大小（对于播放器来说是视频的分辨率）的比例与 ASS 的 PlayRes 的比例不一致，则会导致渲染时缩放与编辑时不一致。
