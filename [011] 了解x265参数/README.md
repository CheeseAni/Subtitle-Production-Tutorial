...
| cbqpoffs    | 码率控制   | -12..12  | 0   | Chroma Cb QP Offset       | 控制Cb的质量                      | 可以调低Chroma的QP以弥补YUV420导致的Chroma平面画质的损失，当然，这会增加码率，建议[0,-3] |
...
| me          | 运动搜索   | 1..5     | 1   | 选择运动搜索方法           | 不同方法压缩率与速率不同            | 1:dia 2:hex 3:umh 4:star 5:full<br>近期个人测试发现umh完全优于star，特别是高marange时更明显，建议只使用umh |
| merange     | 运动搜索   | 0..32768 | 57  | 运动搜索范围               | 提升压缩率，但边际效应明显          | 不求高压缩率时不宜调整，若降低CTU以求速率，应同步降低；<br>1080p时默认的57算是比较合适 |
...
| weightb     | Coding tool | bool   | 0    | 开启B帧加权预测                         | 提升压缩率 | 这是在原本的预测参数上加权，所以不会影响编码速率，所以必开 |
| deblock     | Loop filter | float, float | 0, 0 | 去块滤镜 | 两个形参分别是范围和阈值，阈值越高强度越大 | 建议画面清晰的动画-1,-1，画面存在色块的话可以自行调整 |
| sao         | Loop filter | bool   | 1    | 采样自适应偏移 | | 想法很好，但实际测试结果很烂，一般必关 |
| open-gop     | 切片决策 | bool      | 1    | 允许I帧可以为非IDR帧  | 略微提升压缩率，略微增加解码压力 | 一些HEVC普及早期的古董版本的解码软件可能不兼容，就像现在的解码器不兼容open-gop的AV1，现在无所谓了，所以开 |
| keyint       | 切片决策 | -1..int   | 250  | 最大GOP帧数 | | 若为-1，无限GOP；过高会导致跳转播放时解码时间爆炸，建议[250,450]，特殊情况下可以通过降低keyint来保证画质(不建议) |
| min-keyint   | 切片决策 | 1..keyint | 25   | 最小GOP帧数 | | 建议[1,4] |
| rc-lookahead | 切片决策 | 1..250    | 20   | 切片类型决策前瞻的帧数 | 提升压缩率，有边际效应，略微降低速率 | 用于判断帧类型和QP等；该值不能低于最大连续B帧数；当值高于最大GOP后，多出的部分没有效果 |
| bframes      | 切片决策 | 0..16     | 4    | 最大连续B帧数         | 提升压缩率，降低速率 | 只要兼容，直接拉满就行 |
